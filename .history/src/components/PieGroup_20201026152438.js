    ...
    return (
        <Fragment>
            ...
            <Circle radius={50} x={width / 2} y={height / 2} fill="transparent"
                ...
                {dataCollection.map((item, idx) => <Pie key={idx} idx={idx} width={width} height={height} radius={width > height ? height / 2 : width / 2} data={item} total={sum} rotation={getRotation(idx, item)} totalInput={dataCollection.length} />)}
                <Group width={width / 2}>
