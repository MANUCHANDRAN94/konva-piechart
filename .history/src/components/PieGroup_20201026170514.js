    ...
    return (
        <Fragment>
            ...
            <Circle radius={50} x={width / 2} y={height / 2} fill="transparent"
                ...
                {dataCollection.map((item, idx) => <Pie key={idx} idx={idx} width={width} height={height} radius={width > height ? height / 2 : width / 3} data={item} total={sum} rotation={getRotation(idx, item)} totalInput={dataCollection.length} />)}
                <Group x={width / 3 * 2} y={height * 2 / 3} width={width / 3}>
                    {dataCollection.map((item, idx) => <Scale key={idx} idx={idx} width={width / 3} height={height} radius={width > height ? height / 2 : width / 3} data={item} totalInput={dataCollection.length} />)}
                </Group>
